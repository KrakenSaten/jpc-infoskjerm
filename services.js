(function initServices() {
  ...
  function getDelayMinutes(expectedTime, aimedTime) {
    if (Number.isNaN(aimedTime.getTime())) {
      return 0;
    }
  ...
  function getDepartureStatus(delayMinutes) {
    if (delayMinutes <= 0) {
      return { label: "I rute", delayed: false };
    }
  ...
  function formatPlatform(call) {
    ...
    const platformBits = [quay?.publicCode, quay?.description].filter(Boolean);
    if (!platformBits.length) {
      return "Plattforminformasjon kommer";
    }
    return `Spor ${platformBits.join(" - ")}`;
  }
  ...
  async function fetchDeparturesData(stopId) {
    ...
    const estimatedCalls = payload?.data?.stopPlace?.estimatedCalls || [];
    return estimatedCalls.map(normalizeDeparture).filter(Boolean).sort((left, right) => left.timestamp - right.timestamp).slice(0, 8);
  }
  ...
  async function fetchWeatherData(coords, getWeatherSummary) {
    ...
    const days = daily.time.map((dateText, index) => ({
    ...
    const hourlyForecast = (hourly?.time || []).map((time, index) => ({
      time,
      label: new Date(time).toLocaleTimeString("no-NO", { hour: "2-digit", minute: "2-digit" }).slice(0, 2),
  ...
  async function fetchNewsData(feedUrl, formatNewsTimestamp) {
    ...
    return Array.from(xml.querySelectorAll("channel > item"))
      ...
      .sort((left, right) => (new Date(right.publishedAt).getTime() || 0) - (new Date(left.publishedAt).getTime() || 0))
      .slice(0, 6)
      .map((item) => ({
        title: item.title,
        link: item.link,
